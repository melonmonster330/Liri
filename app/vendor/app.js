(() => {
  ...
  function useTvRoom({ sb: sb2, mode, song, lyrics, playbackTime, isPaused }) {
    ...
    const sendState = useCallback2(async () => {
    ...
    const connect = useCallback2(async (rawCode) => {
      ...
      setError(null);
      const channel = sb2.channel("cast:" + code, {
    ...
    useEffect5(() => () => {
      if (channelRef.current) sb2.removeChannel(channelRef.current);
    }, [sb2]);
  ...
  function Liri() {
    ...
    const [tvCodeInput, setTvCodeInput] = useState7("");
...
Move closer to your speakers and try again.`);
    ...
    return /* @__PURE__ */ React.createElement("div", {
    ...
    }, cast.connected ? `Playing on ${cast.deviceName || "your TV"}` : tvRoom.connected ? `Connected to Samsung TV \xB7 ${tvRoom.roomCode}` : "Put Liri on the big screen"), /* @__PURE__ */ React.createElement("div", {
      ...
      value: tvCodeInput,
      onChange: (e3) => setTvCodeInput(e3.target.value.replace(/\D/g, "").slice(0, 4)),
      onKeyDown: (e3) => {
        if (e3.key === "Enter") tvRoom.connect(tvCodeInput);
      },
      inputMode: "numeric",
      placeholder: "4-digit code",
      "aria-label": "Samsung TV code",
      ...
      onClick: () => tvRoom.connect(tvCodeInput),
