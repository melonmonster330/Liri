(() => {
  ...
  function artistSortKey(name) {
    ...
    const isPerson = words.length > 1 && words.length <= 3 && !BAND_SIGNALS.test(clean) && (GIVEN_NAMES.has(fold(words[0]).replace(/[^a-z]/g, "")) || /^([a-z]\.)+$/i.test(words[0]));
  ...
  function orderLibrary(lib, recentIds) {
    ...
    const rest = (lib || []).filter((x) => !seen.has(String(x.itunes_collection_id))).sort((a, b) => cmp(artistSortKey(a.artist_name), artistSortKey(b.artist_name)) || cmp(a.artist_name, b.artist_name) || cmp(a.album_name, b.album_name));
    return [...recent, ...rest];
  }
