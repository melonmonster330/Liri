...
function Liri() {
  ...
  const fetchUserLibrary = async (uid, autoSelect = false) => {
    ...
    try {
      const {
        data
      } = await sb.from("user_vinyl_library").select("*").eq("user_id", uid).order("added_at", {
        ascending: false
      });
      const library = data || [];
  ...
  const startListening = async (isAutoAdvance = false) => {
    ...
    attemptLogRef.current = [];
    recognitionWonRef.current = false;
    turntableMatchedIdxRef.current = -1; // clear stale side/track display during new listen
    setError(null);
    setMode("listening");
    setListenProgress(0);
    setListenAttempt(0);
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        // echoCancellation + noiseSuppression off: don't process the music signal.
        // autoGainControl left as default (on): lets the device boost ambient music
        // so ACRCloud gets a loud-enough signal to fingerprint.
        audio: {
      ...
      // ── AnalyserNode for frequency-bar wave visualisation ──
      // Taps the same mic stream as MediaRecorder (no extra permission).
      // On iOS Safari AudioContext + MediaRecorder coexist fine since iOS 15.
      try {
        const actx = new (window.AudioContext || window.webkitAudioContext)();
        // iOS starts AudioContext suspended — resume immediately so the analyser works
        if (actx.state === "suspended") actx.resume().catch(() => {});
        const src = actx.createMediaStreamSource(stream);
        const node = actx.createAnalyser();
        node.fftSize = 256; // 128 frequency bins
        node.smoothingTimeConstant = 0.85; // heavy built-in smoothing — kills frame-to-frame jitter
        src.connect(node);
        analyserNodeRef.current = node;
        audioCtxRef.current = actx;
      } catch (e) {/* fall back to chunk-size proxy level for the wave */}

      // ── Live mic level metering via chunk-size proxy (signal indicator + wave fallback) ──
      let smoothedLevel = 0;
      const stopMetering = () => {
        setAudioLevel(0);
      };
      ...
      // ── Recorder helpers ──────────────────────────────────────────────────────
      // On iOS WKWebView, ondataavailable with a timeslice can fire only ONCE
      // (a long-standing WebKit bug). Instead we use recorder.start() with NO
      // timeslice and call stopAndFlush() at each attempt interval. stop() always
      // fires ondataavailable exactly once with all accumulated audio — reliable
      // on every iOS version. After flushing we immediately restart a fresh
      // recorder on the same stream so recording continues uninterrupted.
      // ─────────────────────────────────────────────────────────────────────────
      let recorder;
      let recorderDataResolve = null;
      const allChunks = [];

      const setupRecorder = rec => {
        rec.ondataavailable = e => {
          if (e.data.size > 0) {
            allChunks.push(e.data);
            // Chunk-size proxy for amplitude — updates on each flush (~every 5–6s)
            // The AnalyserNode handles real-time wave animation between flushes.
            const raw = Math.min(1, e.data.size / 5000);
            smoothedLevel = raw > smoothedLevel ? smoothedLevel + (raw - smoothedLevel) * 0.5 : smoothedLevel + (raw - smoothedLevel) * 0.25;
            setAudioLevel(smoothedLevel);
          }
          // Always resolve so stopAndFlush() unblocks even for empty chunks
          if (recorderDataResolve) { recorderDataResolve(); recorderDataResolve = null; }
        };
      };

      // Stop current recorder and wait for ondataavailable to fire (flushes audio)
      const stopAndFlush = () => new Promise(resolve => {
        if (!recorder || recorder.state !== "recording") { resolve(); return; }
        recorderDataResolve = resolve;
        recorder.stop();
      });

      // Start a fresh recorder on the existing stream (keeps allChunks accumulating)
      const restartRecorder = () => {
        ...
        recorder.start(); // No timeslice — data delivered reliably on stop()
      };

      // Full session teardown: stops mic stream + audio context
      ...
      // Read the actual mimeType from a probe recorder (inactive state is fine for .mimeType).
      // On iOS Safari this may differ from preferredMime (e.g. "audio/mp4;codecs=mp4a.40.2").
      // Strip codec suffix because ACRCloud doesn't recognise the long form.
      const _probe = preferredMime ? new MediaRecorder(stream, { mimeType: preferredMime }) : new MediaRecorder(stream);
      const rawMime = _probe.mimeType || preferredMime || "audio/webm";
      const mimeType = rawMime.split(";")[0].trim();
      // _probe remains inactive and is garbage-collected — no stop() needed

      let attempts = 0;
      let sending = false;
      // In turntable mode, Whisper is the only recogniser — fire at 6s and retry every 6s.
      const turntableInterval = 6000;
      let nextAttemptAt = turntableAlbumRef.current ? turntableInterval : ATTEMPT_INTERVAL;

      restartRecorder(); // begin recording immediately
      recordingStartRef.current = Date.now();
      attemptLogRef.current.push(`format: ${mimeType}`);
      let elapsed = 0;
      let bgWhisperStarted = false;
      progressTimerRef.current = setInterval(async () => {
        elapsed += 100;

        // Fill the progress ring smoothly over the full 30s max listen window
        setListenProgress(Math.min(elapsed / (MAX_ATTEMPTS * ATTEMPT_INTERVAL), 1));
        if (elapsed >= nextAttemptAt && !sending) {
          sending = true;
          attempts++;
          setListenAttempt(attempts);
          nextAttemptAt += turntableAlbumRef.current ? turntableInterval : ATTEMPT_INTERVAL;

          // Flush audio from the current recorder into allChunks, then restart
          // recording immediately so the mic stays open while we do API calls.
          await stopAndFlush();
          if (listenSessionRef.current === session && !recognitionWonRef.current) restartRecorder();

          // ── Background Whisper ──
          // Turntable mode: fire on EVERY attempt (every 6s) — don't wait for previous
          // to finish. Each attempt sends a longer audio blob. First match wins.
          // Non-turntable: fire once at attempt 4 (~20s).
          const hasTurntable = !!turntableAlbumRef.current;
          const shouldFireWhisper = hasTurntable ? attempts >= 1 && !recognitionWonRef.current : attempts === 4 && !bgWhisperStarted;
          if (shouldFireWhisper) {
            bgWhisperStarted = true;
            const bgBlob = new Blob([...allChunks], {
              type: mimeType || "audio/webm"
            });
            tryLyricsTranscription(bgBlob, isAutoAdvance).then(found => {
              bgWhisperStarted = false;
              if (found && listenSessionRef.current === session) {
                clearInterval(progressTimerRef.current);
                cleanupSession();
              }
            });
          ...
          // If background Whisper already won, skip remaining ACRCloud attempts
          if (recognitionWonRef.current) {
            clearInterval(progressTimerRef.current);
            cleanupSession();
            return;
          ...
          // In turntable mode, Whisper + cached LRC is the only recogniser — skip ACR entirely.
          // After 15 attempts (~90s) give up so the mic doesn't run forever.
          if (turntableAlbumRef.current) {
            if (attempts >= 15) {
              clearInterval(progressTimerRef.current);
              cleanupSession();
              handleNoMatch(isAutoAdvance, "turntable_no_match");
              return;
            }
            sending = false;
            return;
          ...
          // If blob is empty, MediaRecorder isn't producing audio
          if (blob.size < 500) {
            attemptLogRef.current.push(`#${attempts} (${attempts * 5}s, ${blobKB}KB): no audio captured`);
            if (attempts >= MAX_ATTEMPTS) {
              clearInterval(progressTimerRef.current);
              cleanupSession();
              handleNoMatch(isAutoAdvance, "whisper_failed");
              return;
            }
            sending = false;
            return;
          ...
          attemptLogRef.current.push(data ? `#${attempts} (${attempts * 5}s, ${blobKB}KB): ACR ${data.status?.code} — ${data.status?.msg || "?"}` : `#${attempts} (${attempts * 5}s, ${blobKB}KB): network error`);

          // Abort if the user restarted while we were awaiting — do NOT clear
          // progressTimerRef here since it now belongs to the new session.
          if (listenSessionRef.current !== session) {
            cleanupSession();
            return;
          }
          // Whisper won the race for this session — stop ACRCloud loop
          if (recognitionWonRef.current) {
            clearInterval(progressTimerRef.current);
            cleanupSession();
            return;
          ...
          // ACRCloud match found — ACRCloud wins the race
          if (data?.status?.code === 0 && data?.metadata?.music?.length > 0) {
            recognitionWonRef.current = true;
            clearInterval(progressTimerRef.current);
            cleanupSession();
          ...
          // All ACRCloud attempts exhausted — wait for background Whisper or give up
          if (attempts >= MAX_ATTEMPTS) {
            clearInterval(progressTimerRef.current);
            setListenAttempt(MAX_ATTEMPTS + 1);
            // Give background Whisper up to 40s to complete before giving up
            const deadline = Date.now() + 40000;
            while (!recognitionWonRef.current && Date.now() < deadline) {
