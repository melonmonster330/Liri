...
function validateSubmission(row, durationMs) {
  ...
  if (rawLrc) {
    ...
    return {
      ok: true,
      lrc: normalized,
      plain: lrcToPlain(normalized),
      signature: signature(lrcToPlain(normalized)),
    };
  }
  ...
  return { ok: true, lrc: null, plain: lines.join("\n"), signature: signature(lines.join("\n")) };
}
...
async function researchBug(bug) {
  ...
  for (const candidate of candidates || []) {
    const checked = validateSubmission(candidate, durationMs);
